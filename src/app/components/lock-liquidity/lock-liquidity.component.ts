...
export class LockLiquidityComponent implements OnInit, OnDestroy {
  ...
  async lockLiquidity(tokenAddress: string, time: number, tokenAmount: number) {
    ...
    try {
      ...
      this.web3Service
        .getLocks()
        .then((r: any) => {
          r.map(async (x: any) => (x._tokenName = await this.getLPTokenName(this.form.controls.lockLiquidityTokenAddress.value)));
          this.myLocks = r;
        })
        .catch((err) => {
          console.log(err);
  ...
  // tslint:disable-next-line:typedef
  mapValue(x: any, inMin: any, inMax: any, outMin: any, outMax: any) {
